...
from glyce.layers.graph_position_embed import GraphPositionEmbedder
...
class GraphTransformer(nn.Module):
    def __init__(self, config, num_labels=4):
        ...
        self.graph_embedder = GraphPositionEmbedder(config.graph_config,config)
        ...
        self.bert_model = self.bert_model.from_pretrained(config.graph_config.bert_model)
        if config.bert_frozen == "true":
            print("!=!"*20)
            print("Please notice that the bert model if frozen")
            print("the loaded weights of models is ")
            print(config.graph_config.bert_model)
            print("!-!"*20)
    ...
    def updateEpoch(self):
        ...
        if self.training_strat=="bert-glyce-joint":
            if self.epoch==1:
                ...
                for param in self.graph_embedder.parameters():
                    param.requires_grad=False
            if self.epoch==6:
                print("\nTraining Graph...")
                for param in self.bert_model.parameters():
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,):
        graph_embed = self.graph_embedder(input_ids, token_type_ids=token_type_ids)
        ...
        return encoded_layers, pooled_output2

    def bertForward(self, input_ids, token_type_ids=None, attention_mask=None,):
        sequence_output, pooled_output = self.bert_model(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True)
        context_bert_output = sequence_output[-1]
        return context_bert_output
    def bertGraphForward(self, input_ids, token_type_ids=None, attention_mask=None,):
        graph_embed = self.graph_embedder(input_ids, token_type_ids=token_type_ids)
        sequence_output, pooled_output = self.bert_model(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True)
        context_bert_output = sequence_output[-1]
        input_features = torch.cat([graph_embed, context_bert_output], -1)
