...
class CharGlyphEmbedding(nn.Module):
    ...
    def __init__(self, model_config, idx2char=None):
        ...
        self.glyph_embeddings = nn.ParameterList([nn.Parameter(multiple_glyph_embeddings(
            ...
            self.config.font_size, self.config.use_traditional, self.config.font_normalize, self.config.font_names, i), requires_grad=False) for i in range(self.config.num_fonts_concat)])
